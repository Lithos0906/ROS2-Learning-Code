...
class FaceDetectNode(Node):
    ...
    def parameters_callback(self, parameters):
        for parameter in parameters:
            ...
            if parameter.name == 'number_of_times_to_unsample':
    ...
    def detect_face_callback(self, request, response):
        if request.image.data:
            ...
            self.get_logger().info(f"image is empty, using the default image.")

        start_time = time.time()
        self.get_logger().info(f"loading image completely, start to recognize.")
        ...
        self.get_logger().info(f"use {response.use_time}s")
